...
namespace fe
{
	namespace Description::Data
	{
		...
		constexpr Primitive PrimitiveInType(Type type)
		{
			...
			if (as_int > 20) return Primitive::Float;
			switch ((as_int - 1) / 4)
			{
		...
		constexpr Structure StructureInType(Type type)
		{
			...
			if (as_int > 20) return Structure::Matrix;
			switch ((as_int - 1) % 4)
			{
			...
			FE_CORE_ASSERT(false, "Failed to detect Description::Structure in Description::Data::Type.");
			return Structure::None;
		}
		...
		constexpr size_t SizeOfType(Type type)
		{
			...
			/*constexpr const static uint8_t SDSizeOfTypeLookupTable[] = {
			0,
			4, 4*2, 4*3, 4*4,
			4, 4*2, 4*3, 4*4,
			4, 4*2, 4*3, 4*4,
