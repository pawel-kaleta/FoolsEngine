...
namespace fe
{
    ...
    {
    public:
        OpenGLFramebuffer(const Description::Framebuffer::Specification& spec);
        virtual ~OpenGLFramebuffer() override;

        virtual void Recreate(const Description::Framebuffer::Specification& spec) override;
        ...
        virtual const Description::Framebuffer::Specification& GetSpecification() const override { return m_Specification; };

        static GLenum FormatToGLinternalFormat(Description::Texture::Format format);
        static GLenum FormatToGLtype(Description::Texture::Format format);
        static GLenum FormatToGLformat(Description::Texture::Format format);

        uint32_t m_ID = 0;
        std::string m_Name;
        Description::Framebuffer::Specification m_Specification;
