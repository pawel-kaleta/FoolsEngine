...
namespace fe
{
	...
	bool OpenGLRenderContext::Create()
	{
		FE_PROFILER_FUNC();
		...
		// window creation
		{
			...
			BaseWindowInvisible = glfwCreateWindow(32, 32, "", NULL, NULL); // arbytrary unimportant resolution - invisible placeholder window

			if (!BaseWindowInvisible)
			{
				FE_CORE_ASSERT(false, "GLFW BaseWindowInvisible creation failed!");
				return false;
			}
